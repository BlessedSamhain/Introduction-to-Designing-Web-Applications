<!DOCTYPE html>
<head>
    <title>YES WE HAVE IT </title>
    <link rel="stylesheet" type = "text/css" href="public/css/style.css">
    <script src="https://kit.fontawesome.com/61cd813530.js" crossorigin="anonymous"></script>
</head>
<body class="yeswehaveit_container">
    <a href = "login"><i class="fa-sharp fa-solid fa-book"></i></a>
        <div class="yeswehaveit_image"><img src="public/img/yeswehaveit_image.svg"></div>
        <div class="yeswehaveit_square">YES WE HAVE IT!</div>
        <button class="addtocart_button"><a href="">add to cart</a></button>
        <button class="gotocart_button"><a href="cart">go to cart</a></button>
        <a href="whatyouarelooking" class="yeswehaveit_back"><img src="public/img/back.svg"></a>
</body>